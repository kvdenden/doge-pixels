...
contract BridgedPX is Initializable, ERC721Upgradeable {
    ...
    function __PX_init(
        ...
        // start with no available tokens. They will be made available through bridging
    ...
    function bridgePupper(uint256 pupper) public {
        require(_msgSender() == BRIDGE, "Invalid sender");
        require(pupper >= INDEX_OFFSET && pupper < INDEX_OFFSET + SHIBA_WIDTH * SHIBA_HEIGHT, "Invalid pupper");
        require(pupperToIndex[pupper] == MAGIC_NULL, "Pupper already bridged");

        uint256 LAST_INDEX = INDEX_OFFSET + puppersRemaining;
        uint256 tmpPupper = indexToPupper[LAST_INDEX];

        if (tmpPupper != MAGIC_NULL) {
            // move first unavailable pupper to the back
            uint256 freeIndex = INDEX_OFFSET + totalSupply;
            pupperToIndex[tmpPupper] = freeIndex;
            indexToPupper[freeIndex] = tmpPupper;
        }

        // insert bridged pupper at the end of the `available` pool
    ...
    function _burn(uint256 pupper) internal virtual override {
        ...
        require(ERC721Upgradeable.ownerOf(pupper) == _msgSender(), "Pupper is not yours");
